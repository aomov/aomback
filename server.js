...
const eventSchema = new mongoose.Schema({
  ...
  eventType: { type: String, required: true, enum: ['upcoming', 'past'] },
  eventMainPicture: { type: String, required: true },
  eventMainPicturePublicId: { type: String, required: true },
  eventGallery: [{ type: String }],
  eventGalleryPublicIds: [{ type: String }],
  eventVideo: { type: String },
  eventVideoPublicId: { type: String },
  uploadDate: { type: Date, default: Date.now },
}, { timestamps: true });
...
app.post("/events/upload", checkDbConnection, uploadImage.fields([
  ...
  try {
    ...
    if (!req.files || !req.files.mainPicture) {
      return res.status(400).json({ error: "Main picture is required" });
    }

    const mainPictureFile = req.files.mainPicture[0];
    ...
    const newEvent = new Event({
      ...
      eventMainPicture: mainPictureFile.path,
      eventMainPicturePublicId: mainPictureFile.filename,
      eventGallery: galleryFiles.map(file => file.path),
      eventGalleryPublicIds: galleryFiles.map(file => file.filename),
    });
...
app.post("/events/:id/video", checkDbConnection, uploadVideo.single("video"), async (req, res) => {
  console.log('🎥 Event video upload request');
...
app.put("/events/:id", checkDbConnection, uploadImage.fields([
  ...
  try {
    ...
    if (eventDescription) event.eventDescription = eventDescription.trim();
    ...
    if (req.files && req.files.mainPicture) {
      ...
      const mainPictureFile = req.files.mainPicture[0];
      event.eventMainPicture = mainPictureFile.path;
      event.eventMainPicturePublicId = mainPictureFile.filename;
    }

    // Update gallery if new images are uploaded
    if (req.files && req.files.gallery) {
      // Delete old gallery images
      for (const publicId of event.eventGalleryPublicIds) {
        await safeCloudinaryDestroy(publicId);
      }
      
      const galleryFiles = req.files.gallery;
      event.eventGallery = galleryFiles.map(file => file.path);
      event.eventGalleryPublicIds = galleryFiles.map(file => file.filename);
    }
...
const server = app.listen(PORT, () => {
  ...
  console.log(' Partners: POST/GET/PUT/DELETE /partners');
  console.log(' Reviews: POST/GET/PUT/DELETE /reviews');
  console.log(' Events: POST/GET/PUT/DELETE /events');
  console.log(' Event Video: POST /events/:id/video');
